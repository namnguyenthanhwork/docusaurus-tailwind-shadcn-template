...
// The blog post metadata is serialized to JSON (via actions.createData in the
// blog plugin), so `date` comes back as a string instead of a `Date`
type RecentPostMetadata = Omit<BlogPostMetadata, 'date'> & { date: string }

export interface RecentPost {
  blogData: {
    metadata: RecentPostMetadata
  }
  Preview: unknown
}

export interface HomePageBlogMetadata {
  blogTitle: string
...
function RecentBlogPostCard({ recentPost }: { recentPost: RecentPost }): ReactNode {
      ...
      <CardContent className='p-6'>
        ...
        <div className='*:data-[slot=avatar]:ring-background flex items-center -space-x-2 *:data-[slot=avatar]:size-12 *:data-[slot=avatar]:ring-2 *:data-[slot=avatar]:grayscale'>
          ...
          <div className='ml-4 text-sm dark:text-gray-400'>
            ...
            <span>{Math.ceil(blogData.metadata.readingTime ?? 0)} min read</span>
          </div>
        </div>
      </CardContent>
    </Card>
